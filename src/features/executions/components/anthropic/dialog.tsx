...
export const AnthropicDialog =({open,onOpenChange, onSubmit, defaultValues={}}:Props) =>{
    ...
    return(
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent>
                <DialogHeader>
                    <DialogTitle className="text-primary">Anthropic Configuration</DialogTitle>
                    <DialogDescription className="text-sm text-black ">
                        Configure the AI model and prompts for this Anthropic node.
                    </DialogDescription>
                </DialogHeader>
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-8 mt-4">
                        ...
                        <FormField control={form.control} name="systemPrompt" render={({field}) =>(
                           <FormItem>
                            ...
                            <FormControl>
                                <Textarea {...field} placeholder="You are a helpful assistant" className="min-h-12.5 font-mono text-sm "/>
                            </FormControl>
                            <FormDescription>
                                Sets the behavior of the assistant. Use{"{{variables}}"} for simple values or {"{{json variables}}"} for stringify objects.
                            </FormDescription>
                            <FormMessage/>
                        </FormItem>
                        )}/>
                        <FormField control={form.control} name="userPrompt" render={({field}) =>(
                           <FormItem>
                            ...
                            <FormControl>
                                <Textarea {...field} placeholder="Summarize this text: {{jsonHttpResponse.data}}" className="min-h-20 font-mono text-sm "/>
                            </FormControl>
                            <FormDescription>
                                The prompt to send to AI. Use {"{{variables}}"} for simple values or {"{{json variables}}"} for stringify objects.
                            </FormDescription>
                            <FormMessage/>
                        </FormItem>
                        )}/>
